...
function format(vulnerabilityReport) {
  ...
  if (vulnerabilityTotalCount === 0) {  //eslint-disable-line no-magic-numbers
    ...
    if (vulnerabilityFailCount) {
      returnString += chalk.red(`${vulnerabilityFailCount} failures`);
    } else {
      returnString += chalk.yellow(`${vulnerabilityFailCount} failures`);
    }
    returnString += chalk.yellow(`, ${vulnerabilityWarnCount} warnings).\n`);
  }
